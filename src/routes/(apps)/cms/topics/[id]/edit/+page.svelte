<script>
    ...
    function handleSubmit(e) {
        e.preventDefault();
        console.log('Update topic:', topicId, formData);
    }

    function handleDelete() {
        if (confirm('정말 이 토픽을 삭제하시겠습니까?')) {
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl">
        <header class="px-6 py-4 border-b border-gray-100 dark:border-gray-800">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-lg font-medium text-gray-800 dark:text-gray-100">토픽 수정</p>
                    <p class="text-sm text-gray-500">ID: {topicId}</p>
                </div>
            </div>
        </header>
